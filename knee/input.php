 ...
 *
 * Внимание: $_POST и $_GET объединены в одном массиве $_REQUEST
 * См. /knee/start.php :: ini_set('request_order', 'GP');
 */
    ...
    public static function get($key, $default = null)
    {
        ...
        if (($match = static::parse_key($key)) !== false) {
            if ($match['type'] == 'array')
            {
                ...
                if ($match['wait'] == 'array' AND !is_array($value)) {
                    return $default;
                }
                if ($match['wait'] == 'string' AND !is_string($value)) {
                    return $default;
                }
            }
    ...
    public static function exists($key)
    {
        ...
        if (($match = static::parse_key($key)) !== false)
        {
            if ($match['type'] == 'array') {
                ...
                if ($match['wait'] == 'array' AND ! is_array($link)) {
                    return false;
                }
                if ($match['wait'] == 'string' AND ! is_string($link)) {
                    return false;
                }
